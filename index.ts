...
```
*/
const constructUrl = (base: string | URL, path = "", query: string | Record<string, unknown> | URLSearchParams = ""): string => {
	...
	try {
		...
		if (query) {
			const queryParams = new URLSearchParams(query as string | URLSearchParams | Record<string, string>);
